...
public class AttendanceService {
    ...
    private RecordView recordView(SignRecord r,Long viewerId){User u=users.findById(r.getUserId()).orElseThrow();String photo=r.getPhotoMediaId()==null?null:assets.findById(r.getPhotoMediaId()).map(asset->media.view(asset,viewerId)).map(MediaService.MediaView::url).orElse(null);return new RecordView(r.getId(),r.getTaskId(),r.getUserId(),u.getNickname(),r.getLatitude(),r.getLongitude(),r.getAddress(),r.getRemark(),photo,r.getSignedAt());}
    ...
    public record SignRecordCreated(Long recordId,Long taskId,Long userId){}
