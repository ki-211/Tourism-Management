  ...
  <view class="card activity" @click="$emit('open', activity.id)">
    ...
    <view class="content">
      ...
      <view class="meta-row">
        ...
        <text v-else class="deadline">报名至 {{ displayTime(activity.signupEnd) }}</text>
      </view>
    </view>
  </view>
</template>
...
.visibility { position: absolute; top: 20rpx; right: 20rpx; padding: 8rpx 16rpx; color: #fff; font-size: 22rpx; font-weight: 650; line-height: 1.3; white-space: nowrap; background: rgba(20, 42, 39, .64); border-radius: 999rpx; }
