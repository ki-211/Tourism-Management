...
const _sfc_main = {
  name: "u-upload",
  mixins: [common_vendor.mpMixin, common_vendor.mixin, common_vendor.mixinUpload, common_vendor.props$11],
  data() {
    return {
      lists: [],
      isInCount: true,
...
if (!Array) {
  ...
  (_easycom_u_icon2 + _component_up_icon + _easycom_u_loading_icon2 + _component_up_gap + _component_up_popup)();
}
  ...
  return common_vendor.e({
    ...
    b: common_vendor.f($data.lists, (item, index, i0) => {
      return common_vendor.e({
        ...
        x: common_vendor.t(item.isVideo || item.type && item.type === "video" ? item.name || "视频" : item.name || "文件"),
    ...
    r: common_vendor.o((...args) => $options.loadedVideoMetadata && $options.loadedVideoMetadata(...args))
  } : {}, {
    s: common_vendor.o(($event) => $data.popupShow = $event),
    t: common_vendor.p({
      mode: "center",
      show: $data.popupShow
    }),
