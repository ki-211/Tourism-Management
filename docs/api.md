...
基础路径为 `/api/v1`。除注册、登录、刷新、公开封面、短时签名媒体和健康检查外均需 `Authorization: Bearer <accessToken>`。
...
- `GET|PATCH /users/me`，`PUT /users/me/password`，`POST /users/me/logout-all`，`DELETE /users/me`
- `GET /users/me/sign-records`
...
- `GET|POST /activities/{id}/photos`，`DELETE /activities/{id}/photos/{photoId}`
...
- `GET /media/public/{id}`（仅活动封面），`GET /media/access/{id}?token=...`（5 分钟签名地址）
